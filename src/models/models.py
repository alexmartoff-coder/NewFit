...
professional_specializations = Table(
    ...
    Base.metadata,
    Column("professional_id", ForeignKey("professional_profiles.id"), primary_key=True),
...
class User(Base):
    ...
    professional_profile: Mapped["ProfessionalProfile"] = relationship(back_populates="user", cascade="all, delete-orphan")
    ...
    schedule = relationship("ProfessionalSchedule", back_populates="professional", uselist=False)
...
class ProfessionalProfile(Base):
    ...
    specializations: Mapped[List[Specialization]] = relationship(
        secondary=professional_specializations, backref="professionals"
    )
    subscriptions: Mapped[List["Subscription"]] = relationship(back_populates="professional")
...
class Subscription(Base):
    ...
    professional: Mapped["ProfessionalProfile"] = relationship(back_populates="subscriptions")
...
class Admin(Base):
    ...
    role = Column(String(50), default="admin")  # "owner", "co_admin", "tester_professional", "tester_client", "tester_both"
    added_by = Column(BigInteger, nullable=True)
    added_at = Column(DateTime, default=datetime.utcnow)
    can_test_professional = Column(Boolean, default=False)
...
class ProfessionalSchedule(Base):
    ...
    id = Column(Integer, primary_key=True)
    professional_id = Column(BigInteger, ForeignKey("users.id"), nullable=False, unique=True)
    ...
    professional = relationship("User", back_populates="schedule")
...
class ProfessionalTemplate(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    professional_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.id"))
...
class TimeSlot(Base):
    ...
    professional_profile_id = Column(Integer, ForeignKey("professional_profiles.id", ondelete="CASCADE"), nullable=False)
    ...
    professional_profile = relationship("ProfessionalProfile", backref="time_slots")
...
class Booking(Base):
    ...
    professional_profile: Mapped["ProfessionalProfile"] = relationship(back_populates="bookings")
