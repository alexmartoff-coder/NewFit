        ...
        if len(unique_clients) >= 10 and not is_sub_valid:
            ...
            if profile.subscription_expires_at and profile.subscription_expires_at <= datetime.now(timezone.utc).replace(tzinfo=None):
                text_sub = (
                    ...
                    "Ваша ежемесячная подписка NewFit (10 ₽/мес) закончилась.\n"
                ...
                text_sub = (
                    ...
                    "Чтобы продолжить работать со своей базой клиентов и совершать повторные записи, "
                    "вам необходимо оформить подписку NewFit за **10 ₽/мес**."
                )
            kb_sub = types.InlineKeyboardMarkup(
                inline_keyboard=[
                    [types.InlineKeyboardButton(text="💳 Оплатить подписку 10 ₽", callback_data="pay_sub_4990")]
                ]
            )
            if isinstance(event, types.CallbackQuery):
                if message.photo:
        ...
        if unique_clients:
            ...
            for c in unique_clients:
                ...
                kb_list.append([types.InlineKeyboardButton(
                    text=f"Забронировать время для {client_name}",
