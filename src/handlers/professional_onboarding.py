...
from src.keyboards.common import get_format_kb, get_professional_main_kb, get_start_reg_kb, get_spec_kb, get_city_kb, get_client_main_kb
from src.keyboards.inline import add_admin_button
from src.models.models import User, ProfessionalProfile, UserRole, WorkFormat, Specialization, Booking
...
async def start_reg(callback: types.CallbackQuery, state: FSMContext, effective_user_id: int = None, is_admin: bool = False):
    ...
    await state.set_state(ProfessionalOnboarding.full_name)
...
async def skip_step_handler(callback: types.CallbackQuery, state: FSMContext, is_admin: bool = False, effective_user_id: int = None):
    ...
    async with SessionLocal() as session:
        user = await session.get(User, user_id)
        stmt = select(ProfessionalProfile).where(ProfessionalProfile.user_id == user_id).options(selectinload(ProfessionalProfile.specializations))
        res = await session.execute(stmt)
        profile = res.scalar_one_or_none()

        if current_state == ProfessionalOnboarding.full_name:
            ...
            await state.set_state(ProfessionalOnboarding.city)
            kb = get_city_kb()
        ...
        elif current_state == ProfessionalOnboarding.city:
            await state.update_data(city=profile.city)
            await state.set_state(ProfessionalOnboarding.specialization)
        ...
        elif current_state == ProfessionalOnboarding.experience:
            ...
            if data.get('role') == UserRole.BEAUTY:
                ...
                await state.set_state(ProfessionalOnboarding.price_services)
                ...
                if specs:
                    ...
                    await state.set_state(ProfessionalOnboarding.price_package)
                    await callback.message.answer("Шаг 7/9\n\nУкажите цену за пакет услуг (в ₽):")
            else:
                await state.set_state(ProfessionalOnboarding.formats)
                kb = get_format_kb()
                skip_kb = types.InlineKeyboardMarkup(inline_keyboard=[
        ...
        elif current_state == ProfessionalOnboarding.price_single:
        ...
        elif current_state == ProfessionalOnboarding.price_package:
            await state.update_data(price_package=profile.price_package)
            await state.set_state(ProfessionalOnboarding.photo)
            skip_kb = types.InlineKeyboardMarkup(inline_keyboard=[
        ...
        elif current_state == ProfessionalOnboarding.photo:
            await state.update_data(photo_url=profile.photo_url)
            await state.set_state(ProfessionalOnboarding.video)
            kb = types.InlineKeyboardMarkup(inline_keyboard=[
...
@router.message(ProfessionalOnboarding.full_name)
...
@router.message(ProfessionalOnboarding.city)
...
@router.callback_query(F.data.startswith("spec_"), ProfessionalOnboarding.specialization)
async def process_spec_callback(callback: types.CallbackQuery, state: FSMContext, is_admin: bool = False, effective_user_id: int = None):
    if callback.data == "spec_done":
        ...
        user_id = effective_user_id or callback.from_user.id
        await state.set_state(ProfessionalOnboarding.experience)
...
@router.message(ProfessionalOnboarding.experience)
...
@router.callback_query(F.data.startswith("fmt_"), ProfessionalOnboarding.formats)
async def process_formats_callback(callback: types.CallbackQuery, state: FSMContext, is_admin: bool = False, effective_user_id: int = None):
    ...
    await state.set_state(ProfessionalOnboarding.price_single)
...
@router.message(ProfessionalOnboarding.price_services)
...
@router.message(ProfessionalOnboarding.price_single)
...
@router.message(ProfessionalOnboarding.price_package)
...
@router.message(ProfessionalOnboarding.photo, F.photo)
...
@router.callback_query(F.data == "skip_video", ProfessionalOnboarding.video)
...
@router.message(ProfessionalOnboarding.video, F.video)
...
async def finish_onboarding(message: types.Message, state: FSMContext, user_id: int, username: str, is_admin: bool = False):
    try:
        ...
        async with SessionLocal() as session:
            ...
            # === 2. ProfessionalProfile — просто обновляем, без удаления ===
            stmt = select(ProfessionalProfile).where(ProfessionalProfile.user_id == user_id).options(selectinload(ProfessionalProfile.specializations))
            res = await session.execute(stmt)
            professional_profile = res.scalar_one_or_none()
            ...
            if not professional_profile:
                ...
                session.add(professional_profile)
                ...
                professional_profile.experience = int(data.get('experience', professional_profile.experience))
                professional_profile.work_format = data.get('work_format', professional_profile.work_format)
                professional_profile.price_single = float(data.get('price_single', professional_profile.price_single))
                professional_profile.price_package = float(data.get('price_package', professional_profile.price_package))
                professional_profile.service_prices = data.get('service_prices', professional_profile.service_prices)
                if photo_url:
                    professional_profile.photo_url = photo_url
                if video_url:
                    professional_profile.video_presentation_url = video_url
                professional_profile.status = "approved"
                logger.info(f"Обновлён существующий профиль профессионала {user_id}")

            # Flush to get the professional_profile.id if it's new, required for specializations association
            await session.flush()
            ...
            if data.get('specializations'):
                ...
                professional_profile.specializations = list(spec_res.scalars().all())
                logger.info(f"Linked {len(professional_profile.specializations)} specializations for professional {user_id}")
        ...
        await message.answer(
            f"Поздравляем! 🎉\n\n{role_success_text}",
            reply_markup=get_professional_main_kb(is_admin=is_admin)
        )
