        ...
        if "postgresql" in str(engine.url).lower():
            ...
            async with engine.begin() as conn:
                ...
                async def add_column_safe_local(c, table, col_name, col_type):
                    try:
                        await c.execute(text(f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS {col_name} {col_type}"))
                    except Exception as e:
                ...
                await add_column_safe_local(conn, "time_slots", "trainer_profile_id", "INTEGER")
                await add_column_safe_local(conn, "bookings", "trainer_profile_id", "INTEGER")
                await add_column_safe_local(conn, "trainer_schedules", "trainer_id", "BIGINT")
                await add_column_safe_local(conn, "schedule_templates", "trainer_id", "BIGINT")
                ...
                # Прочие колонки
                await add_column_safe_local(conn, "bookings", "slot_id", "INTEGER")
                await add_column_safe_local(conn, "bookings", "client_id", "BIGINT")
                await add_column_safe_local(conn, "bookings", "start_time", "TIMESTAMP WITHOUT TIME ZONE")
                await add_column_safe_local(conn, "bookings", "end_time", "TIMESTAMP WITHOUT TIME ZONE")
                await add_column_safe_local(conn, "bookings", "status", "VARCHAR(50)")
                await add_column_safe_local(conn, "bookings", "price", "FLOAT")
                await add_column_safe_local(conn, "bookings", "paid", "BOOLEAN DEFAULT FALSE")
                await add_column_safe_local(conn, "bookings", "client_notes", "TEXT")
                await add_column_safe_local(conn, "bookings", "trainer_notes", "TEXT")
                await add_column_safe_local(conn, "bookings", "booked_at", "TIMESTAMP WITHOUT TIME ZONE DEFAULT NOW()")

                await add_column_safe_local(conn, "client_profiles", "full_name", "VARCHAR(128)")
                await add_column_safe_local(conn, "client_profiles", "status", "VARCHAR(20) DEFAULT 'active'")

                await add_column_safe_local(conn, "reviews", "booking_id", "INTEGER")


                # trainer_schedules columns
                for col in ["google_client_id", "google_client_secret", "google_calendar_id"]:
                    await add_column_safe_local(conn, "trainer_schedules", col, "VARCHAR(200)")
                for col in ["google_refresh_token", "google_access_token"]:
                    await add_column_safe_local(conn, "trainer_schedules", col, "TEXT")
                await add_column_safe_local(conn, "trainer_schedules", "token_expires_at", "TIMESTAMP WITHOUT TIME ZONE")
                await add_column_safe_local(conn, "trainer_schedules", "sync_enabled", "BOOLEAN DEFAULT TRUE")
                await add_column_safe_local(conn, "trainer_schedules", "timezone", "VARCHAR(50) DEFAULT 'Europe/Moscow'")
                await add_column_safe_local(conn, "trainer_schedules", "slot_duration", "INTEGER DEFAULT 60")
                ...
                await add_column_safe_local(conn, "trainer_profiles", "price_online", "FLOAT DEFAULT 0.0")
                await add_column_safe_local(conn, "trainer_profiles", "price_package", "FLOAT DEFAULT 0.0")
                ...
                await add_column_safe_local(conn, "trainer_profiles", "online_meeting_link", "VARCHAR(500)")
                ...
                # time_slots extra
                await add_column_safe_local(conn, "time_slots", "format", "VARCHAR(100) DEFAULT 'hybrid'")
                try:
                    await conn.execute(text("ALTER TABLE time_slots ALTER COLUMN format TYPE VARCHAR(100)"))
                except Exception: pass

                await add_column_safe_local(conn, "time_slots", "google_event_id", "VARCHAR(200)")
                await add_column_safe_local(conn, "time_slots", "zoom_meeting_id", "VARCHAR(100)")
                await add_column_safe_local(conn, "time_slots", "zoom_join_url", "VARCHAR(500)")
                await add_column_safe_local(conn, "time_slots", "zoom_start_url", "VARCHAR(500)")
                await add_column_safe_local(conn, "time_slots", "online_platform", "VARCHAR(50)")
                await add_column_safe_local(conn, "time_slots", "max_clients", "INTEGER DEFAULT 1")
                await add_column_safe_local(conn, "time_slots", "notes", "TEXT")
